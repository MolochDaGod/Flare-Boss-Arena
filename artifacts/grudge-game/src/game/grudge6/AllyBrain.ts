...
export interface AllyAgent {
  ...
  respawnAt: number;
  hurtFlash: number;
  /** Runtime override from AI HUD (harvest / defend / follow). */
  brainOverride?: AllyBrainId | null;
}

/** Resolve active brain (HUD override wins). */
export function activeBrain(agent: AllyAgent): AllyBrainId {
  return agent.brainOverride ?? agent.instance.def.brain;
}
...
export function thinkAlly(
  ...
  // Auto-harvest: gatherer always; bodyguard/patrol when no combat focus
  const wantsHarvest =
    brain === "gatherer" ||
    ((brain === "bodyguard" || brain === "patrol") && !world.focusEnemyId);
  if (wantsHarvest && world.harvest.length) {
    // Threat interrupt: defend player first if enemies close
    if (brain === "gatherer" || brain === "bodyguard") {
      let nearThreat: (typeof world.enemies)[0] | null = null;
      let td = 9;
      for (const e of world.enemies) {
        const d = e.pos.distanceTo(world.playerPos);
        if (d < td) {
      ...
      if (nearThreat && td < 8) {
        const dist = nearThreat.pos.distanceTo(agent.pos);
        if (dist > kit.attackRange * 0.92) {
          agent.state = "attack";
          return { type: "move", targetPos: nearThreat.pos.clone() };
        }
        if (agent.attackCd <= 0) {
          agent.state = "attack";
          agent.attackCd = kit.attackCd;
          return { type: "attack", enemyId: nearThreat.id, targetPos: nearThreat.pos.clone() };
        }
        agent.state = "attack";
        return { type: "idle" };
      }
    ...
    // Gatherer may roam farther; bodyguard stays near player corridor
    const maxNodeDist = brain === "gatherer" ? 22 : 14;
    const maxPlayerCorridor = brain === "gatherer" ? 28 : 16;
    if (nd < maxNodeDist && nearest.pos.distanceTo(world.playerPos) < maxPlayerCorridor) {
      if (nd > GATHER_RANGE) {
        agent.state = "gather";
  ...
  // Defend stance (bodyguard): tight formation hold when idle
  if (brain === "bodyguard") {
    agent.state = "follow";
    const form = world.playerPos.clone().add(allyFormationOffset(agent.followSlot, playerFacing));
    if (agent.pos.distanceTo(form) > FOLLOW_DIST * 0.35) {
