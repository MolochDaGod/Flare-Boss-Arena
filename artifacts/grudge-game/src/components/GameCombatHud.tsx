...
export interface GameCombatHudProps {
  ...
  raceClass: string;
  skillBar: Array<
    | { id: string; name: string; icon?: string; glyph?: string; pending?: boolean }
    | undefined
  >;
  skillCdPct?: number[];
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-2 items-end px-3 pb-3">
        ...
        {/* Skill bar — single production HUD (glyph + key), frost chrome */}
          ...
          <div className="pointer-events-auto flex gap-1.5">
            {skillBar.slice(0, 5).map((sk, i) => (
              <button
                ...
                className="relative w-11 h-11 rounded-lg text-[10px] font-mono tracking-wide transition-transform hover:-translate-y-0.5 flex flex-col items-center justify-center"
                style={{
                  background: sk?.pending
                    ? "rgba(40,80,120,0.55)"
                    : "rgba(13,23,34,0.72)",
                  border: sk?.pending
                    ? "1px solid rgba(100,200,255,0.75)"
                    : "1px solid rgba(120,190,230,0.35)",
                  color: "#eaf3fe",
                  boxShadow: sk?.pending ? "0 0 12px rgba(100,200,255,0.35)" : "0 0 8px rgba(80,160,200,0.1)",
                }}
              >
                <span className="text-[9px] absolute top-0.5 left-1 text-[#9ab0c6]">{i + 1}</span>
                <span className="text-base leading-none">{sk?.glyph ?? "·"}</span>
                <span className="text-[7px] truncate max-w-[40px] text-[#9ab0c6]">
                  {(sk?.name ?? "").split(" ")[0]}
                </span>
              </button>
            ))}
            <button
              ...
              onClick={() => onSkill?.(-1)}
              title="Special (R)"
              className="w-11 h-11 rounded-lg text-[10px] font-mono flex flex-col items-center justify-center"
              style={{
                background: "rgba(83,221,176,0.12)",
                border: "1px solid rgba(83,221,176,0.55)",
                color: "#53ddb0",
                opacity: 0.5 + 0.5 * specialReadyPct,
              }}
            >
              <span className="text-sm">R</span>
              <span className="text-[7px]">SPEC</span>
            </button>
          </div>
          <p className="text-[10px] text-[#9ab0c6] text-center max-w-[300px] leading-snug">
            WASD · LMB move · RMB attack · Shift dodge · Q block · 1–5 skills · R special
          </p>
        </div>
