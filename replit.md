...
- **Dungeon scale + terrain** (`GameEngine.ts`, `proceduralTextures.ts`) — `DUNGEON=90` drives a ~180×180 square. The `forge-scene.glb` dungeon model (via `DungeonMap`) was REMOVED at user request (it sat centered at origin); `loadEnvironment()` now just marks the map ready and `dungeonMap` stays null, so all `dungeonMap?.ready` branches fall back to the flat procedural floor + square clamp (see `.agents/memory/dungeon-map-removed.md`). Runtime-canvas cobblestone ground, `makeRockField` InstancedMesh scatter, `makeTerrainSkirt` (Chebyshev mask matches the square movement clamp), ACES tone mapping, player-following sun/shadow rig, hover raycasting.
...
- **ARPG skill/combat framework** (`game/combat/*`) — scene-agnostic combat layer shared by all 3 scenes: archetype shapes (circle/cone/line/nova) + deployables (fire_totem/turret/trap), XZ-plane `pointInShape`/`targetsInShape` damage math, native-shader `TelegraphField` ground decals, element-aware three.quarks `ParticleVfx` (fire/ice/lightning/poison/arcane/physical — 4 textures + per-element palette + shape-distinct `castSkillVfx`; GLB cloud kept for nova/circle via `skillVfx`), dual aiming (auto-aim default + cursor-aim on mouse-hold, dungeon only). Dungeon has persistent deployables; Camp/Arena downgrade deployables to an instant nova pulse. Pages feed resolved class skills via `setHudSkills`. See `.agents/memory/combat-skill-framework.md`.
