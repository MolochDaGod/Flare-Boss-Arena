...
- **Dungeon scale + terrain** (`GameEngine.ts`, `proceduralTextures.ts`) — `DUNGEON=90` drives a ~180×180 square; the `forge-scene.glb` dungeon is scaled to `targetExtent=130` (must stay proportional to `DUNGEON`; pirate `coveCenter.x` sits just past the forge edge but inside `DUNGEON` — see `.agents/memory/dungeon-scale-coupling.md`). Runtime-canvas cobblestone ground, `makeRockField` InstancedMesh scatter, `makeTerrainSkirt` (Chebyshev mask matches the square movement clamp), ACES tone mapping, player-following sun/shadow rig, hover raycasting.
